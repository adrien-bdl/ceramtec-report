...
This report therefore delivers three things: (i)~a critical reframing of the client's problem; (ii)~a step-by-step pricing framework with limitations stated explicitly; and (iii)~a maintainable data pipeline (register, scraping/processing scripts, merged external dataset) that CeramTec can extend after the project. Key learnings include the heterogeneity of regulated healthcare markets and the use of AI-assisted extraction to scale retrieval from unstructured PDF reports---with the caveat that data comparability remains uneven across countries and OEMs.
...
The objective of this project is to assess the feasibility of a \textbf{data-driven pricing framework} for CeramTec's ceramic hip bearings and to deliver a reproducible evidence base that the company can maintain after the consulting engagement. A central finding is that the client's initial ``dynamic pricing'' framing must be reinterpreted: what CeramTec needs is not minute-by-minute price optimisation but \textbf{stratified negotiation baselines} grounded in market, payer, and account context. Section~\ref{sec:reframing} develops this argument; Sections~\ref{sec:data-constraints}--\ref{sec:methodology} present constraints, mitigations, and our four-phase methodology; Section~\ref{sec:learnings} summarises what we learned from the healthcare market and from building AI-assisted data pipelines.
...
The project objective is to design a \textbf{data-driven price stratification framework} for CeramTec's hip implant bearing components: moving from cost-plus intuition to evidence-backed \textbf{baseline price ranges} that support OEM negotiations by market and account profile. The deliverable is not a live dynamic-pricing engine but a structured methodology, data register, and reproducible pipeline that CeramTec can operationalise once internal and external data are fully integrated.
...
A core deliverable beyond static analysis is a \textbf{maintainable external-data capability}: reproducible scraping/processing scripts, a synchronized team register (Google Sheet $\rightarrow$ repo), one EDA notebook per data category, and a merged external dataset (\texttt{data/external/external\_data\_dataset.csv}).


\section{Reframing: Price Stratification versus Dynamic Pricing}
\label{sec:reframing}
...
\section{Data Landscape and Project Constraints}
\label{sec:data-constraints}

\subsection{Internal data}

Until the project, CeramTec had not used data systematically to support pricing. Internal sources include:

\begin{itemize}
  \item \textbf{Salesforce} --- records demand signals and client meetings, but \textbf{no revenue data}; it cannot on its own support price--volume analysis.
  \item \textbf{SAP} --- transactional detail including pieces sold (daily timestamps), revenue, customer name, material number and name, product group, and geographic fields (to be interpreted cautiously because OEM corporations operate internationally).
\end{itemize}

A major integration barrier is that \textbf{Salesforce and SAP cannot be directly linked}: no common key exists between the two environments, limiting unified account-level feature engineering until CeramTec defines a master customer identifier.

\textbf{NDA and timing.} Historical sales and pricing data existed at CeramTec throughout the project, but contractual clearance delayed access. We obtained internal data only on \textbf{7 July 2026}, leaving limited time for full integration into Phases~2--4 within the academic timeline. Spencer committed to sharing supplementary files (accounts summary and SAP revenue data) to support ongoing work.

Preliminary inspection of SAP series reveals observable patterns relevant to pricing segmentation: seasonal drops in \textbf{August and December} (production and demand), \textbf{June spikes} linked to grouped orders from Chinese customers, and a \textbf{2023 COVID-related surge} followed by a decrease in 2024. These patterns motivate time-aware features in Phase~2 even though full modelling was not completed before the report deadline.

\subsection{External data}

With limited internal history at project start, we invested heavily in \textbf{external evidence}: reimbursement ceilings, procurement outcomes, registry bearing mix, procedure volumes, macro cost indices, and OEM financial signals. Healthcare markets are \textbf{highly regulated} and \textbf{country-specific}---US DRG episode payments, French GHS and LPPR device lists, Italy's regionalised DRG and prosthetic tariffs, and China's NHSA volume-based procurement (VBP) operate on different logic. Few open, machine-readable datasets exist; much information sits in lengthy PDF reports from agencies, registries, and industry bodies with non-standard layouts.

\subsection{Implication for project deliverables}

The combination of late internal access, sparse external APIs, and cross-country heterogeneity means the full end-to-end pricing model \textbf{cannot be completed within the project window}. Our deliverable is therefore the \textbf{complete methodological framework} (Phases~1--4), a fully implemented Phase~1 pipeline, explicit documentation of limitations, and reproducible artefacts (register, scripts, notebooks) so CeramTec can continue execution.


\section{Mitigation: AI-Assisted Data Retrieval}
\label{sec:mitigation}

To scale collection of unstructured external data, we developed \textbf{AI-assisted scraping and processing scripts} that:
\begin{itemize}
  \item automatically retrieve reports and datasets from public portals and registries;
  \item extract tables and indicators from PDF documents (e.g.\ RIAP national registry reports, AJRR summaries, Open LPP reimbursement files);
  \item harmonise outputs into tidy CSVs under \texttt{data/external/<category>/processed/}.
\end{itemize}

This agentic, script-based approach materially improves throughput compared with manual copy-paste from PDFs. However, it is \textbf{not a turnkey solution}:
\begin{itemize}
  \item extracted values require \textbf{manual validation}, especially when report formats change between years or when addenda supersede main tables;
  \item there is \textbf{no guarantee of parity} across countries or OEMs---empirically, data richness varies sharply (strong US/IT registry extracts versus partial China proxies);
  \item maintenance effort grows with each new source format.
\end{itemize}

We consider this the most pragmatic path for CeramTec given industry data opacity, but it reinforces that price stratification must be built on \textbf{transparent quality flags} rather than assumed completeness.


\section{Proposed Four-Phase Methodology}
\label{sec:methodology}

Our pricing framework proceeds in four phases. Table~\ref{tab:phases} summarises objectives and status; subsequent sections detail each phase and what we would have delivered with complete, timely data.
...
3 & Market and customer segmentation & Cluster OEMs and markets (e.g.\ K-means on financials, innovation, reimbursement pressure); define stratification tiers --- \textbf{designed, not fully executed} \\
4 & Price baseline model & Predict anchor price ranges per segment; negotiation playbooks --- \textbf{designed, not fully executed} \\
\bottomrule
\end{tabular}
\end{table}

Phase~1 established the reproducible backbone: without a maintained register and automated retrieval, any segmentation model would age quickly as reimbursement rules, VBP rounds, and registry publications change. Phases~2--4 depend on linking CeramTec internal bearing price history to the external panel via a resolved customer master key.
...
This project began with a client request for ``dynamic pricing'' and concludes with a more accurate diagnosis: CeramTec needs \textbf{data-driven price stratification}---evidence-backed baseline ranges for a finite set of strategic OEM relationships across regulated national markets. We showed that literal dynamic pricing is mismatched to the B2B component-supplier context.

We delivered a complete \textbf{four-phase framework}, fully implemented \textbf{Phase~1} (external register, automated retrieval, exploratory analysis, merged external dataset), and documented why Phases~2--4 could not be finished: late internal data access (7 July 2026), missing Salesforce--SAP linkage, and the inherent difficulty of harmonising healthcare data across countries. We also demonstrated a scalable mitigation path via AI-assisted extraction, with transparent limits on cross-country comparability.

For CeramTec, the recommended next steps are: (i)~establish a customer master key and integrate Spencer's SAP revenue and accounts data; (ii)~refresh the external register on a schedule; (iii)~execute segmentation and baseline models within the Phase~3--4 design; and (iv)~use stratified bands in negotiations rather than pursuing real-time dynamic repricing. The scripts, register, and notebooks in this repository are intended to make that continuation straightforward.
